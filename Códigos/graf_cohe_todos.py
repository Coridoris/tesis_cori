# -*- coding: utf-8 -*-
"""
Created on Fri Nov 17 13:12:17 2023
...
#%% acomodo los datos para coherencia promedio
...
coherence_camp = coherence1[1]

#%%grafico coherencia promedio vs cada x cantidad de oraciones
...
for i in range(len(temas)):
    tema =  temas[i]
    path1 = f'C:/Users/Usuario/Desktop/Cori/Tesis/{entrevista}_entrevista/Coherencia/{tema}_coherencia_evolucion.csv'
    path2 = f'C:/Users/Usuario/Desktop/Cori/Tesis/{entrevista}_entrevista/Coherencia/{tema}_coherencia_evolucion_norm.csv'
    
    df_coherence_evolution = pd.read_csv(path2)
    df_coherence_evolution_ = pd.read_csv(path1)
    
    # Convertir los valores de la columna coherencia_evolucion de cadena a listas
    df_coherence_evolution_['coherencia_evolucion'] = df_coherence_evolution_['coherencia_evolucion'].apply(ast_literal_eval_notnan)
    ...
    coherence1.append([v[0] if type(v) == list else np.nan for v in df_coherence_evolution['coherencia_evolucion']])
    coherence2.append([v[1] if type(v) == list else np.nan for v in df_coherence_evolution['coherencia_evolucion']])
...
ax1.axvline(x =q_modelo_nulo[13], color='black', linestyle='--', linewidth = 4.5,  label = 'cuantil 95% sujeto')
ax1.axvline(x = q_coherence_rand_1, color='black', linestyle='-', linewidth = 4.5, label = 'cuantil 95% prom')

ax1.hist(modelo_nulo[13], edgecolor='black', color = "mediumseagreen", alpha=0.5, label = "Modelo nulo de un sujeto")
ax1.hist(coherence1_random, edgecolor='black', color = "sandybrown", alpha=0.5, label = "Modelo nulo promedio")
ax1.tick_params(labelsize =15)

ax1.axvline(x = coherence1[0][13], color='midnightblue', linestyle='--', linewidth = 4.5,  label = 'coherencia sujeto camp')

ax1.axvline(x = coherence_camp , color='midnightblue', linestyle='-', linewidth = 4.5, label= "Coherencia promedio camp")
